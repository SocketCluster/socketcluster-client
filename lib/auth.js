var AuthEngine = function () {
  ...
  this.isLocalStorageEnabled = this._isLocalStorageEnabled();
};
...
AuthEngine.prototype._listenToLocalStorageKeyChanges = function (name) {
  var self = this;
...
AuthEngine.prototype._stopListeningToLocalStorageKeyChanges = function (name) {
  var listener = this._localStorageChangeHandlers[name];
  if (listener) {
    global.removeEventListener('storage', listener);
    delete this._localStorageChangeHandlers[name];
  }
};
...
AuthEngine.prototype.watchToken = function (name, watcher) {
  if (this.isLocalStorageEnabled && global.localStorage) {
    this._listenToLocalStorageKeyChanges(name);
  }
  this._addTokenChangeWatcher(name, watcher);
};

AuthEngine.prototype.unwatchToken = function (name, watcher) {
  this._removeTokenChangeWatcher(name, watcher);
  if (this.isLocalStorageEnabled && global.localStorage && !this._hasTokenChangeWatchers(name)) {
    this._stopListeningToLocalStorageKeyChanges(name);
  }
};
