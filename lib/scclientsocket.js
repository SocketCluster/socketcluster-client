...
SCClientSocket.prototype.deauthenticate = function () {
  return this.auth.removeToken(this.authTokenName)
  .then((oldToken) => {
  ...
  .then((result) => {
    ...
    // TODO 2: Replace with this.emit
    Emitter.prototype.emit.call(this, 'removeAuthToken', result.oldToken);
...
SCClientSocket.prototype.authenticate = function (signedAuthToken) {
  return this.emit('#authenticate', signedAuthToken)
  .then((authStatus) => {
  ...
  .then((result) => {
    ...
    return this.auth.saveToken(this.authTokenName, signedAuthToken, {})
    .then(() => {
      return null;
    })
    .catch((err) => {
...
SCClientSocket.prototype.publish = function (channelName, data) {
  ...
  return this.emit('#publish', pubData);
};
