...
const ASTransport = require('./astransport').ASTransport;
...
function ASClientSocket(socketOptions) {
...
ASClientSocket.prototype = Object.create(AsyncStreamEmitter.prototype);

ASClientSocket.CONNECTING = ASClientSocket.prototype.CONNECTING = ASTransport.prototype.CONNECTING;
ASClientSocket.OPEN = ASClientSocket.prototype.OPEN = ASTransport.prototype.OPEN;
ASClientSocket.CLOSED = ASClientSocket.prototype.CLOSED = ASTransport.prototype.CLOSED;

ASClientSocket.AUTHENTICATED = ASClientSocket.prototype.AUTHENTICATED = 'authenticated';
ASClientSocket.UNAUTHENTICATED = ASClientSocket.prototype.UNAUTHENTICATED = 'unauthenticated';

ASClientSocket.SUBSCRIBED = ASClientSocket.prototype.SUBSCRIBED = SCChannel.SUBSCRIBED;
ASClientSocket.PENDING = ASClientSocket.prototype.PENDING = SCChannel.PENDING;
ASClientSocket.UNSUBSCRIBED = ASClientSocket.prototype.UNSUBSCRIBED = SCChannel.UNSUBSCRIBED;

ASClientSocket.ignoreStatuses = scErrors.socketProtocolIgnoreStatuses;
ASClientSocket.errorStatuses = scErrors.socketProtocolErrorStatuses;

// TODO 2: Test this in the browser
// TODO 2: Check that there are no memory leaks
ASClientSocket.prototype._handleBrowserUnload = async function () {
  let unloadHandler = () => {
    this.disconnect();
  };
...
ASClientSocket.prototype._privateDataHandlerMap = {
...
ASClientSocket.prototype._privateRPCHandlerMap = {
...
ASClientSocket.prototype.getState = function () {
  return this.state;
};

ASClientSocket.prototype.getBytesReceived = function () {
  return this.transport.getBytesReceived();
};

ASClientSocket.prototype.deauthenticate = async function () {
  (async () => {
    let oldAuthToken;
    try {
      oldAuthToken = await this.auth.removeToken(this.authTokenName);
...
ASClientSocket.prototype.connect = function () {
  if (this.state === this.CLOSED) {
    this.pendingReconnect = false;
    this.pendingReconnectTimeout = null;
    ...
    let transport = new ASTransport(this.auth, this.codec, this.options);
...
ASClientSocket.prototype.reconnect = function (code, data) {
  this.disconnect(code, data);
  this.connect();
};

ASClientSocket.prototype.disconnect = function (code, data) {
  code = code || 1000;
...
ASClientSocket.prototype._changeToUnauthenticatedStateAndClearTokens = function () {
...
ASClientSocket.prototype._changeToAuthenticatedState = function (signedAuthToken) {
...
ASClientSocket.prototype.decodeBase64 = function (encodedString) {
  let decodedString;
...
ASClientSocket.prototype.encodeBase64 = function (decodedString) {
  let encodedString;
  if (typeof Buffer === 'undefined') {
    if (global.btoa) {
...
ASClientSocket.prototype._extractAuthTokenData = function (signedAuthToken) {
...
ASClientSocket.prototype.getAuthToken = function () {
  return this.authToken;
};

ASClientSocket.prototype.getSignedAuthToken = function () {
  return this.signedAuthToken;
};

// Perform client-initiated authentication by providing an encrypted token string.
ASClientSocket.prototype.authenticate = async function (signedAuthToken) {
  let authStatus;
...
ASClientSocket.prototype._tryReconnect = function (initialDelay) {
...
ASClientSocket.prototype._onSCOpen = function (status) {
  ...
  if (status) {
    ...
    // This can happen if auth.loadToken (in astransport.js) fails with
...
ASClientSocket.prototype._onSCError = function (error) {
  this.emit('error', {error});
};

ASClientSocket.prototype._suspendSubscriptions = function () {
...
ASClientSocket.prototype._abortAllPendingEventsDueToBadConnection = function (failureType, donePromise) {
...
ASClientSocket.prototype._onSCClose = function (code, reason, openAbort) {
  this.id = null;
  if (this.transport) {
    this.transport.closeAllListeners();
  }
  ...
  if (!ASClientSocket.ignoreStatuses[code]) {
    let closeMessage;
    if (reason) {
    ...
    let err = new SocketProtocolError(ASClientSocket.errorStatuses[code] || closeMessage, code);
    this._onSCError(err);
  }
};

ASClientSocket.prototype._onSCInboundTransmit = function (event, data) {
...
ASClientSocket.prototype._onSCInboundInvoke = function (event, data, response) {
...
ASClientSocket.prototype.decode = function (message) {
  return this.transport.decode(message);
};

ASClientSocket.prototype.encode = function (object) {
  return this.transport.encode(object);
};

ASClientSocket.prototype._flushTransmitBuffer = function () {
  let currentNode = this._transmitBuffer.head;
  let nextNode;
...
ASClientSocket.prototype._handleEventAckTimeout = function (eventObject, eventNode) {
  if (eventNode) {
    eventNode.detach();
  }
  delete eventObject.timeout;
...
ASClientSocket.prototype._processOutboundEvent = function (event, data, expectResponse) {
  if (this.state === this.CLOSED) {
    this.connect();
  }
  let eventObject = {
...
ASClientSocket.prototype.send = function (data) {
  this.transport.send(data);
};

ASClientSocket.prototype.transmit = function (event, data) {
  return this._processOutboundEvent(event, data);
};

ASClientSocket.prototype.invoke = function (event, data) {
  return this._processOutboundEvent(event, data, true);
};

ASClientSocket.prototype.publish = function (channelName, data) {
  let pubData = {
...
ASClientSocket.prototype._triggerChannelSubscribe = function (channel, subscriptionOptions) {
...
ASClientSocket.prototype._triggerChannelSubscribeFail = function (err, channel, subscriptionOptions) {
  let channelName = channel.name;
...
ASClientSocket.prototype._cancelPendingSubscribeCallback = function (channel) {
...
ASClientSocket.prototype._decorateChannelName = function (channelName) {
...
ASClientSocket.prototype._undecorateChannelName = function (decoratedChannelName) {
...
ASClientSocket.prototype._trySubscribe = function (channel) {
  let meetsAuthRequirements = !channel.options.waitForAuth || this.authState === this.AUTHENTICATED;
...
ASClientSocket.prototype.subscribe = function (channelName, options) {
  options = options || {};
  let channel = this._channelMap[channelName];
...
ASClientSocket.prototype._triggerChannelUnsubscribe = function (channel, setAsPending) {
  let channelName = channel.name;
...
ASClientSocket.prototype._tryUnsubscribe = function (channel) {
  if (this.state === this.OPEN) {
    let options = {
      noTimeout: true
    };
...
ASClientSocket.prototype.unsubscribe = function (channelName) {
  let channel = this._channelMap[channelName];
...
ASClientSocket.prototype.channel = function (channelName) {
...
ASClientSocket.prototype.getChannelState = function (channelName) {
  let channel = this._channelMap[channelName];
  if (channel) {
...
ASClientSocket.prototype.getChannelOptions = function (channelName) {
  let channel = this._channelMap[channelName];
  if (channel) {
...
ASClientSocket.prototype.receiver = function (receiverName) {
  return this._receiverDemux.stream(receiverName);
};

ASClientSocket.prototype.closeReceiver = function (receiverName) {
  this._receiverDemux.close(receiverName);
};

ASClientSocket.prototype.procedure = function (procedureName) {
  return this._procedureDemux.stream(procedureName);
};

ASClientSocket.prototype.closeProcedure = function (procedureName) {
  this._procedureDemux.close(procedureName);
};

ASClientSocket.prototype.closeChannel = function (channelName) {
  this._channelDataDemux.close(channelName);
};

ASClientSocket.prototype.subscriptions = function (includePending) {
  let subs = [];
  Object.keys(this._channelMap).forEach((channelName) => {
...
ASClientSocket.prototype.isSubscribed = function (channelName, includePending) {
  let channel = this._channelMap[channelName];
  if (includePending) {
    return !!channel;
  }
...
ASClientSocket.prototype.processPendingSubscriptions = function () {
  this.preparingPendingSubscriptions = false;
  let pendingChannels = [];
