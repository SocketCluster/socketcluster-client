...
var SCSocket = function (opts) {
  ...
  // TODO: Should we add an authStateChange event?
  // TODO: Should we add a subscribeStateChange event? Also emit on channel?
...
SCSocket.prototype._privateEventHandlerMap = {
  '#fail': function (data) {
    var error = this.transport.hydrateError(data);
  ...
  '#removeAuthToken': function (data, response) {
    ...
    this.auth.removeToken(this.options.authTokenName, function (err, oldToken) {
      if (err) {
        ...
        self._changeToUnauthenticatedState(oldToken);
        response.end();
      }
    });
  },
...
SCSocket.prototype._changeToUnauthenticatedState = function (signedAuthToken) {
  if (this.authState != 'unauthenticated') {
    this.authState = 'unauthenticated';
    SCEmitter.prototype.emit.call(this, 'deauthenticate', signedAuthToken);
  }
};
...
SCSocket.prototype.authenticate = function (signedAuthToken, callback) {
  ...
  this.emit('#authenticate', signedAuthToken, function (err, authStatus) {
    if (err) {
      self._changeToUnauthenticatedState(signedAuthToken);
...
SCSocket.prototype._onSCOpen = function (status) {
  ...
  if (status) {
    ...
    if (status.isAuthenticated) {
      ...
      // TODO: Should deauthenticate be triggered when changing from pending to unauthenticated? (upon initial connection).
      this._changeToUnauthenticatedState(status.authToken);
    }
  } else {
    this._changeToUnauthenticatedState(status.authToken);
  }
