...
SCSocket.prototype._onSCOpen = function () {
  ...
  this._sendCurrentAuthToken(function (err) {
    if (err) {
      self._onSCError(err);
      self._onSCClose(4004);
      self.socket.close(4004);
    } else {
      self._resubscribe();
      ...
      clearTimeout(self._initTimeoutTicker);
      
      // In case 'status' event isn't triggered on time
      self._initTimeoutTicker = setTimeout(function () {
...
SCSocket.prototype._sendCurrentAuthToken = function (callback) {
  var self = this;
  
  this.auth.loadToken(this.options.authTokenName, function (err, token) {
    if (!err) {
      // If this fails, the server will not send back a #status event and this will
      // cause the client socket to emit an error
      self._emitDirect('#authToken', token);
    }
    callback(err);
  });
};
        ...
        if (eventName == '#fail') {
          ...
          if (tokenData) {
            ...
            var triggerSetAuthToken = function (err) {
              if (err) {
                ...
                Emitter.prototype.emit.call(self, 'setAuthToken', tokenData.token);
                response.end();
              }
            };
          ...
          this.auth.removeToken(this.options.authTokenName, function (err) {
            if (err) {
              ...
              Emitter.prototype.emit.call(self, 'removeAuthToken');
              response.end();
            }
          });
