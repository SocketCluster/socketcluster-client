...
var SCSocket = function (opts) {
  ...
  if (this.options.watchAuthToken && this.auth.watchToken) {
    this._authTokenWatcher = function (tokenValue) {
      if (tokenValue == null) {
        self.deauthenticate();
      } else {
        if (self.state == self.CLOSED) {
          self._changeToPendingAuthState();
        } else {
          // Authenticate with new token.
          self.authenticate(tokenValue);
        }
      }
    };
    this.auth.watchToken(this.authTokenName, this._authTokenWatcher);
  }
...
SCSocket.prototype.connect = SCSocket.prototype.open = function () {
  ...
  if (this.state == this.CLOSED) {
    ...
    // this._changeToPendingAuthState(); // TODO 2
    ...
    this.transport.on('handshakeStart', function (authToken) {
      if (authToken == null) {
        self._changeToUnauthenticatedState();
      } else {
        self._changeToPendingAuthState();
...
SCSocket.prototype.destroy = function () {
  ...
  if (this._authTokenWatcher) {
    this.auth.unwatchToken(this.authTokenName, this._authTokenWatcher);
  }
  this.disconnect();
};
...
SCSocket.prototype.authenticate = function (signedAuthToken, callback) {
  ...
  if (signedAuthToken == null) {
    var err = new InvalidArgumentsError('Failed to authenticate because signedAuthToken was null or undefined');
    var authStatus = {
      authError: err,
