...
var SCSocket = function (opts) {
  ...
  if(typeof Promise !== "undefined" && Promise.toString().indexOf("[native code]") !== -1 && opts.usePromises){
...
SCSocket.prototype.emit = function (event, data, callback) {
  var self = this
  if (callback) {
    if (self._localEvents[event] == null) {
      self._emit(event, data, callback);
    } else {
      Emitter.prototype.emit.call(self, event, data);
    }
  }

  if (self.allowPromises) {
    return new Promise(function(resolve,reject) {
      self._emit(event, data, function(err,res) {
