...
SCSocket.prototype._privateEventHandlerMap = {
  ...
  '#setAuthToken': function (data, response) {
    ...
    if (data) {
      ...
      if (data.persistent && data.expiresInMinutes != null) {
        ...
        this.auth.saveToken(this.options.authTokenName, tokenData.token, tokenOptions, triggerSetAuthToken);
      }
    } else {
  ...
  '#disconnect': function (data) {
    this._onSCClose(data.code, data.data);
  }
};
...
SCSocket.prototype.connect = SCSocket.prototype.open = function () {
  if (this.state == this.CLOSED) {
    ...
    if (this.transport) {
      this.transport.socket = null;
    }
    this.transport = new SCTransport(this);
  }
};
...
SCSocket.prototype.disconnect = function (code, data) {
  ...
  if (this.state == this.OPEN) {
    ...
    this.transport.emitDirect('#disconnect', packet);
...
SCSocket.prototype._onSCOpen = function (status) {
  ...
  // TODO: REMOVE: this._resetPingTimeout();
};
...
SCSocket.prototype._onSCClose = function (code, data) {
  ...
  /*
  TODO: Move this logic to transport
  ...
  */
  ...
  if (this.state != this.CLOSED) {
    ...
    if (oldState == this.OPEN || code == 1000) {
      setTimeout(function () {
        SCEmitter.prototype.emit.call(self, 'disconnect', code, data);
      }, 0);
    } else if (oldState == this.CONNECTING) {
      setTimeout(function () {
        SCEmitter.prototype.emit.call(self, 'connectFail', code, data);
      }, 0);
    }
...
SCSocket.prototype.stringify = function (object) {
  return this.transport.stringify(this._convertBuffersToBase64(object));
};
...
SCSocket.prototype._emit = function (event, data, callback) {
  ...
  if (callback) {
    eventObject.timeout = setTimeout(function () {
      // TODO: Maybe the transport should handle the timeout, maybe we can
      // expose a function to explicitly add the callback to the _callbackMap inside the transport
      // from the socket
...
// TODO: We need a method on transport instead of delete this._callbackMap[channel._pendingSubscriptionCid];
// TODO: Should the socket know about the cid??
...
SCSocket.prototype._trySubscribe = function (channel) {
  ...
  if (this.state == this.OPEN && channel._pendingSubscriptionCid == null) {
    ...
    channel._pendingSubscriptionCid = this.transport.emitDirect(
...
SCSocket.prototype._tryUnsubscribe = function (channel) {
  ...
  if (this.state == this.OPEN) {
    ...
    this.transport.emitDirect('#unsubscribe', channel.name, options);
  }
};
