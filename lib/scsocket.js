...
SCSocket.prototype._trySubscribe = function (channel) {
  ...
  var isAuthReady = !channel.waitForAuth || this.authState == 'authenticated';

  // We can only ever have one pending subscribe action at any given time on a channel
  if (this.state == this.OPEN && !this.pendingConnectCallback &&
    channel._pendingSubscriptionCid == null && isAuthReady) {
...
SCSocket.prototype.subscribe = function (channelName, options) {
  ...
  if (options && options.waitForAuth) {
    channel.waitForAuth = true;
  }
