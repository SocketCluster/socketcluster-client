...
SCSocket.prototype._onSCOpen = function () {
  ...
  this._handshake(function (err, status) {
    // If the underlying socket has changed, do nothing
    if (socket == self.socket) {
      if (err) {
        self._onSCError(err);
        self._onSCClose(4003);
        ...
        if (status) {
          self.id = status.id;
          self.pingTimeout = status.pingTimeout;
        }
        
        SCEmitter.prototype.emit.call(self, 'connect', status);
...
SCSocket.prototype._handshake = function (callback) {
  var self = this;
...
SCSocket.prototype._onSCMessage = function (message) {
  ...
  SCEmitter.prototype.emit.call(this, 'message', message);
    ...
    if (eventName) {
      ...
      if (this._isPrivateTransmittedEvent(eventName)) {
        if (eventName == '#fail') {
          ...
          if (channel) {
            SCEmitter.prototype.emit.call(this, 'kickOut', eventData.message, channelName);
          ...
          if (tokenData) {
            var tokenOptions = {
              ...
              expiresInMinutes: !!tokenData.expiresInMinutes
            };
            ...
            var triggerSetAuthToken = function (err) {
              if (err) {
                ...
                SCEmitter.prototype.emit.call(self, 'setAuthToken', tokenData.token);
                response.end();
              }
            };
          ...
          var response = new Response(this, obj.cid);
          SCEmitter.prototype.emit.call(this, eventName, eventData, function (error, data) {
      ...
      SCEmitter.prototype.emit.call(this, 'raw', obj);
    }
  }
};
