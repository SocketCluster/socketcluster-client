  ...
  if (this.state != this.CLOSED) {
    ...
    if (this.options.autoReconnect) {
      if (code == 4000) {
        ...
        // In case of ping timeout, we delete the processing
        // flag from pending events to allow them to be retried
        // once the socket reconnects (if it manages to do so before
        // the event times out).
        var currentNode = this._emitBuffer.head;
        
        while (currentNode) {
          delete currentNode.processing;
...
SCSocket.prototype._flushEmitBuffer = function (socket) {
  ...
  while (currentNode) {
    if (!currentNode.processing) {
      var eventObject = currentNode.data;
      
      // Volatile event can be removed from emitBuffer 
      // immediately - If it fails, we don't need to 
      // try to emit again
      if (!eventObject.callback) {
        currentNode.detach();
      }
...
SCSocket.prototype._emit = function (socket, event, data, callback) {
  ...
  if (this._localEvents[event] == null) {
    ...
    if (callback) {
      eventObject.callback = function (err) {
        eventNode.detach();
        callback.apply(eventObject, arguments);
      };
      ...
      if (!this._persistentEvents[event]) {
        eventObject.timeout = setTimeout(function () {
          ...
          var cb = eventObject.callback;
          delete eventObject.callback;
          eventNode.detach();
          cb.call(eventObject, error, eventObject);
